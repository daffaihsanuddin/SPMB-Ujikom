...
@section('title', 'Statistik Wilayah Pendaftar')
@section('content')
    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="d-flex justify-content-between align-items-center">
                                <small class="text-muted">
                                    Menampilkan:
                                    <span id="showingCount">{{ $statistik->count() }}</span> dari
                                    <span id="totalCount">{{ $statistik->count() }}</span> wilayah
                                </small>
            ...
            <div class="card bg-primary text-white">
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                        <div>
                            <h4 id="totalWilayahCard">{{ $statistik->count() }}</h4>
        ...
        <div class="col-md-3 mb-3">
            <div class="card bg-success text-white">
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                        <div>
                            <h4 id="totalPendaftarCard">{{ $statistik->sum('total') }}</h4>
        ...
        <div class="col-md-3 mb-3">
            <div class="card bg-info text-white">
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                        <div>
                            <h4 id="rataRataCard">{{ number_format($statistik->avg('total'), 1) }}</h4>
        ...
        <div class="col-md-3 mb-3">
            <div class="card bg-warning text-dark">
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                        <div>
                            <h4 id="wilayahTerbanyakCard">{{ $statistik->max('total') }}</h4>
                        ...
                        <table class="table table-striped" id="statistikTable">
                            ...
                            <tbody id="tableBody">
                                ...
                                @foreach($statistik as $index => $item)
                                    <tr class="wilayah-row" data-kecamatan="{{ strtolower($item->kecamatan) }}"
                                        data-kelurahan="{{ strtolower($item->kelurahan) }}" data-total="{{ $item->total }}">
                                        <td>{{ $index + 1 }}</td>
                                        <td>
                                            <strong class="kecamatan-text">{{ $item->kecamatan }}</strong>
                                        </td>
                                        <td class="kelurahan-text">{{ $item->kelurahan }}</td>
                                        <td class="text-end">
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-sm btn-outline-info" data-bs-toggle="tooltip"
                                                ...
                                                onclick="showWilayahDetail('{{ $item->kecamatan }}', '{{ $item->kelurahan }}')">
                                                <i class="fas fa-search"></i>
                                            </button>
                                        </td>
                                    </tr>
                            ...
                            <tfoot>
                                <tr class="fw-bold">
                                    ...
                                    <td class="text-end" id="footerTotal">{{ $totalAll }}</td>
        ...
        <div class="col-md-4 mb-4">
                ...
                <div class="card-body">
                    <div id="searchSummary">
                        <div class="d-flex justify-content-between mb-2">
                            ...
                            <strong id="foundCount">{{ $statistik->count() }}</strong>
                        </div>
                        <div class="d-flex justify-content-between mb-2">
                            <small class="text-muted">Total pendaftar:</small>
                            <strong id="foundTotal">{{ $statistik->sum('total') }}</strong>
                        </div>
                        <div class="d-flex justify-content-between">
                            <small class="text-muted">Rata-rata:</small>
                            <strong id="foundAverage">{{ number_format($statistik->avg('total'), 1) }}</strong>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body" id="topWilayahChart">
                    @foreach($statistik->take(5) as $index => $item)
                        <div class="mb-3">
                            <div class="d-flex justify-content-between mb-1">
                                <small class="text-muted">
                                    {{ $index + 1 }}. {{ $item->kecamatan }}
                                </small>
                                <small class="text-muted">{{ $item->total }} pendaftar</small>
                            </div>
...
@push('scripts')
    <script>
        let allData = {!! json_encode($statistik) !!};
        ...
        function exportToPDF() {
            ...
            let url = '{{ route("admin.export.statistik-wilayah-pdf") }}';
