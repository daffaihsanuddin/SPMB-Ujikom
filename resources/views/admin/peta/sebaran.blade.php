                        ...
                        <div class="col-12">
                            <div class="d-flex flex-wrap gap-3">
                                <div class="d-flex align-items-center">
                                    <span class="badge bg-primary me-2" id="totalMarkers">{{ count($dataMap) }}</span>
                                    <small class="text-muted">Total Pendaftar</small>
                                </div>
                                <div class="d-flex align-items-center">
                                    <span class="badge bg-success me-2" id="visibleMarkers">{{ count($dataMap) }}</span>
                ...
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <small class="text-muted" id="tableCount">
                            Menampilkan {{ count($dataMap) }} data
                        </small>
                    </div>
                </div>
                <div class="card-body">
                        ...
                        <table class="table table-sm table-hover" id="mapDataTable">
                            ...
                            <tbody id="tableBody">
                                @foreach($dataMap as $item)
                                    <tr class="map-data-row" data-nama="{{ strtolower($item['nama']) }}"
                                        data-jurusan="{{ strtolower($item['jurusan']) }}" data-status="{{ $item['status'] }}"
                                        data-alamat="{{ strtolower($item['alamat']) }}" data-lat="{{ $item['lat'] }}"
                                        data-lng="{{ $item['lng'] }}">
                                        <td>
                                            <strong>{{ $item['nama'] }}</strong>
                                        </td>
                                        <td>
                                            <span class="badge bg-primary">{{ $item['jurusan'] }}</span>
                                        </td>
                                        <td>
                                            @php
                                                $statusClass = [
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-sm btn-outline-info"
                                                onclick="zoomToMarker({{ $item['lat'] }}, {{ $item['lng'] }}, '{{ $item['nama'] }}')">
...
@push('scripts')
    ...
    <script>
        ...
        function initializeMarkers() {
            ...
            @foreach($dataMap as $item)
                const marker{{ $loop->index }} = L.marker([{{ $item['lat'] }}, {{ $item['lng'] }}], {
                    icon: L.divIcon({
                        className: 'custom-marker',
                        html: `<div style="background-color: ${statusColors['{{ $item['status'] }}']}; 
                        ...
                        iconAnchor: [10, 10]
                                    ...
                                    <div class="text-center">
                                        ...
                                        <p class="mb-1">
                                            <i class="fas ${statusIcons['{{ $item['status'] }}']} me-1"></i>
                                            <span class="badge bg-{{ $statusClass }}">{{ $item['status'] }}</span>
                                        </p>
                                        <p class="mb-1"><strong>{{ $item['jurusan'] }}</strong></p>
                                        <p class="mb-1 text-muted"><small>{{ $item['alamat'] }}</small></p>
                                        <hr class="my-2">
                                        <small class="text-muted">
                ...
                markerCluster.addLayer(marker{{ $loop->index }});
            @endforeach
