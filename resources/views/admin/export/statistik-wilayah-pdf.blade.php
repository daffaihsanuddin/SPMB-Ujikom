...
<body>
    ...
    <div class="filter-info">
        <strong>Filter pencarian:</strong> "{{ $searchTerm }}"
    </div>
    @endif
    ...
    <div class="summary-grid">
        <div class="summary-card">
            <div class="summary-number">{{ $summary['total_wilayah'] }}</div>
            <div class="summary-label">Total Wilayah</div>
        </div>
        <div class="summary-card">
            <div class="summary-number">{{ $summary['total_pendaftar'] }}</div>
            <div class="summary-label">Total Pendaftar</div>
        </div>
        <div class="summary-card">
            <div class="summary-number">{{ $summary['rata_rata'] }}</div>
            <div class="summary-label">Rata-rata per Wilayah</div>
        </div>
        <div class="summary-card">
            <div class="summary-number">{{ $summary['wilayah_terbanyak'] }}</div>
            <div class="summary-label">Wilayah Terbanyak</div>
        </div>
    </div>

    <!-- Top 5 Wilayah -->
    @if($summary['top_5_wilayah']->count() > 0)
    <div class="section-title">5 Besar Wilayah dengan Pendaftar Terbanyak</div>
    <div class="top-wilayah">
        @foreach($summary['top_5_wilayah'] as $index => $item)
        <div class="top-item">
            <div class="top-info">
                <span><strong>{{ $index + 1 }}. {{ $item->kecamatan }} - {{ $item->kelurahan }}</strong></span>
                <span>{{ $item->total }} pendaftar ({{ $item->persentase }}%)</span>
            </div>
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $summary['total_pendaftar'] > 0 ? round(($data->sum('total') / $summary['total_pendaftar']) * 100, 1) : 0 }}%</td>
            </tr>
            @endforeach
        </tbody>
    </table>
