...
<body>
    <div class="header">
        ...
        <p>Periode: {{ $filterInfo['start_date'] ? date('d/m/Y', strtotime($filterInfo['start_date'])) : 'Semua' }} - 
           {{ $filterInfo['end_date'] ? date('d/m/Y', strtotime($filterInfo['end_date'])) : 'Semua' }}</p>
    </div>
    ...
    <div class="info-box">
        <div class="info-row">
            ...
            <span class="info-value">{{ $filterInfo['jurusan'] }}</span>
        </div>
            <div class="info-row">
            <span class="info-label">Gelombang:</span>
            <span class="info-value">{{ $filterInfo['gelombang'] }}</span>
        </div>
        <div class="info-row">
            <span class="info-label">Tanggal Cetak:</span>
            <span class="info-value">{{ $filterInfo['tanggal_cetak'] }}</span>
        </div>
        <div class="info-row">
    ...
    <div style="display: flex; gap: 10px; margin-bottom: 15px;">
        <div style="flex: 1; background: #28a745; color: white; padding: 10px; border-radius: 5px; text-align: center;">
            <div style="font-size: 16px; font-weight: bold;">Rp {{ number_format($statistik['total_pendapatan'], 0, ',', '.') }}</div>
        ...
        <div style="flex: 1; background: #17a2b8; color: white; padding: 10px; border-radius: 5px; text-align: center;">
            <div style="font-size: 16px; font-weight: bold;">{{ $statistik['total_peserta'] }}</div>
        ...
        <div style="flex: 1; background: #ffc107; color: black; padding: 10px; border-radius: 5px; text-align: center;">
            <div style="font-size: 16px; font-weight: bold;">Rp {{ number_format($statistik['rata_rata'], 0, ',', '.') }}</div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-right">Rp {{ number_format($item->gelombang->biaya_daftar, 0, ',', '.') }}</td>
            ...
            <tr class="summary-row">
                ...
                <td class="text-right"><strong>Rp {{ number_format($statistik['total_pendapatan'], 0, ',', '.') }}</strong></td>
    ...
    <div style="margin-top: 20px;">
        ...
        <table>
            ...
            <tbody>
                ...
                @foreach($jurusanSummary as $summary)
                <tr>
                    <td>{{ $summary['nama'] }}</td>
                    <td class="text-center">{{ $summary['count'] }}</td>
                    <td class="text-right">Rp {{ number_format($summary['total'], 0, ',', '.') }}</td>
                    <td class="text-right">Rp {{ number_format($summary['total'] / $summary['count'], 0, ',', '.') }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <div style="margin-top: 20px;">
        ...
        <table>
            ...
            <tbody>
                ...
                @foreach($gelombangSummary as $summary)
                <tr>
                    <td>{{ $summary['nama'] }}</td>
                    <td class="text-center">{{ $summary['count'] }}</td>
                    <td class="text-right">Rp {{ number_format($summary['total'], 0, ',', '.') }}</td>
                    <td class="text-right">Rp {{ number_format($summary['biaya'], 0, ',', '.') }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
